...
public class RectScalerWithViewport : MonoBehaviour
{
    ...
    void UpdateRect()
    {
...
#if UNITY_EDITOR
        if (aspectData.targetResolution.width == 0f || aspectData.targetResolution.height == 0f)
        {
            return;
        }
        Camera cam = Camera.main;
        ...
        // canvas scalerから引用
        float logWidth = Mathf.Log(width / aspectData.targetResolution.width, kLogBase);
        float logHeight = Mathf.Log(height / aspectData.targetResolution.height, kLogBase);
